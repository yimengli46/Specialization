...
class Data_Gen_View:
    ...
    def __init__(self, split, scene_floor_tuple, saved_dir='', hm3d_to_lvis_dict=None,
        ...
        scene_folder = f'{scene_folder}/collected_data/'
        if not os.path.exists(scene_folder):
            os.mkdir(scene_folder)
        ...
        for (s, e) in graph.edges():
            ps = graph[s][e]['pts']
            ps_sparse = ps[NODE_GAP:ps.shape[0]:NODE_GAP, :]
...
output_folder = 'output/COLLECT_VLMAPS_DATA'
...
with open('data/versioned_data/hm3d-1.0/hm3d/hm3d_annotated_basis.scene_dataset_config.json') as f:
    ...
    for json_dir in list_json_dirs:
        ...
        #if sem_filename == '00800-TEEsavR23oF':
...
list_scene_floor_tuple = list_scene_floor_tuple[5:]
