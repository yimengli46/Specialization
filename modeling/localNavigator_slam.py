    ...
    def plan_to_reach_frontier(self, agent_map_pose, chosen_frontier, occ_map):
            ...
            if self.check_drift(agent_map_pose):  # collison
                ...
                for i in range(length):
                    for j in range(width):
                        ...
                        wx = x1 + 0.05 * (i + buf) * np.sin(t1) + \
                            0.05 * (j - width // 2) * np.cos(t1)
                        wy = y1 + 0.05 * (i + buf) * np.cos(t1) - \
                            0.05 * (j - width // 2) * np.sin(t1)
                        cell_coords = pose_to_coords(
                            [wx, wy], self.pose_range, self.coords_range, self.WH)
        ...
        if self.flag_collision:
            if False:
                ...
                ax[0].scatter(agent_coords[0], agent_coords[1],
                              marker=marker, s=(30*scale)**2, c='red', zorder=5)
                ax[1].imshow(traversible, cmap='gray')
                ax[2].imshow(self.collision_map)
                ax[2].scatter(agent_coords[0], agent_coords[1],
                              marker=marker, s=(10*scale)**2, c='red', zorder=5)
                fig.tight_layout()
