...
def compute_frontier_potential(frontiers, occupancy_grid, gt_occupancy_grid, observed_area_flag, sem_map, skeleton=None, unet_model=None, device=None):
    ...
    if cfg.NAVI.PERCEPTION == 'Potential':
            ...
            for f in frontiers:
                if component[int(f.centroid[0]), int(f.centroid[1])]:
                    ...
                    if cfg.NAVI.D_type == 'Sqrt_R':
                        ...
                        # try:
                        cost_dall, cost_din, cost_dout, component_G = skeletonize_frontier_graph(
                            component, skeleton)
                        '''
						except:
        ...
        resized_Mp = np.zeros(
            (2, cfg.PRED.PARTIAL_MAP.INPUT_WH[1], cfg.PRED.PARTIAL_MAP.INPUT_WH[0]), dtype=np.float32)
        resized_Mp[0] = cv2.resize(
            occupancy_grid, cfg.PRED.PARTIAL_MAP.INPUT_WH, interpolation=cv2.INTER_NEAREST)
        resized_Mp[1] = cv2.resize(
            sem_map, cfg.PRED.PARTIAL_MAP.INPUT_WH, interpolation=cv2.INTER_NEAREST)
        ...
        for f in frontiers:
            ...
            if mask_points.shape[0] > 0:
                U_a = max(
                    np.mean(points_vals[mask_points, 0]) * cfg.PRED.PARTIAL_MAP.DIVIDE_AREA, 1.)
