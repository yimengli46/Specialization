'''
compare weighted knowledge graphs with Cosine similarity
...
train_scene_co_matrix_folder = 'output/weighted_kg/train'
...
z_ij_train = np.zeros((num_classes, num_classes, 2), dtype=np.int32)
...
weighted_co_matrix = np.zeros((num_classes, num_classes), dtype=np.float32)
...
np.save(f'output/weighted_kg/z_ij_train_all.npy', z_ij_train)
np.save(f'output/weighted_kg/weighted_co_matrix_train_all.npy', weighted_co_matrix)
'''

z_ij_train = np.load(
    'output/weighted_kg/z_ij_train_all.npy', allow_pickle=True)
weighted_co_matrix_train = np.load(
    'output/weighted_kg/weighted_co_matrix_train_all.npy', allow_pickle=True)

'''
# === for visualize the probability graph between target objects and other objects
row_sum = weighted_co_matrix_train.sum(axis=1)
# maximum_row_idx = np.argmax(row_sum)
maximum_row_idx = 240
...
val_scene_co_matrix_folder = 'output/weighted_kg/val'
comatrix_name_list = [os.path.splitext(os.path.basename(x))[0] for x in sorted(
...
for comatrix_name in comatrix_name_list:
    ...
    if True:
        for portion in portion_list:
            ...
            # weighted_co_matrix_train_portion = weighted_co_matrix_train * \
            #     weighted_co_matrix_val_portion
...
# write html to file
html_f = open(f'output/weighted_kg/Cosine_similarity.html', "w")
