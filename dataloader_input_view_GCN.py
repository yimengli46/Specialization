...
class view_dataset(data.Dataset):
    ...
    def __init__(self, split, scene_name, floor_id=0, data_folder='', hm3d_to_lvis_dict=None,
                 LVIS_dict=None, transform=None, bbox_type='gt', kg_type='binary'):
        ...
        if self.kg_type == 'binary':
            co_matrix_HM3D = np.load(
                'output/kg_per_scene/co_matrix_all_train_scene.npy', allow_pickle=True)
            binary_co_matrix_HM3D = co_matrix_HM3D > 10

            # build the nodes and edges
            nodes = np.zeros((len(self.goal_obj_index_list), 1))  # num_nodes, num_node_features
            self.nodes = nodes

            row_indices, col_indices = np.where(binary_co_matrix_HM3D)
            edge_index = np.stack((row_indices, col_indices))  # 2, num_edges
    ...
    def __getitem__(self, index):
        ...
        if self.bbox_type == 'gt':
            ...
            for hm3d_cat_index in img_hm3d_cat_index_list:
                ...
                # if current category is in the goal category list
                if lvis_cat_name in self.goal_category_set:
                    cat_id = self.goal_obj_index_list.index(self.LVIS_dict['rowid2catid_dict'][self.LVIS_dict['cat_synonyms'].index(
                        lvis_cat_name)])
                    nodes[cat_id, 0] = 1
